...
# ==== causal-conv1d 统一包装（兼容 1.3.x ~ 1.5.x）====
try:
    from causal_conv1d import causal_conv1d_cuda as _ccc  # 优先包命名空间
except Exception:
    import causal_conv1d_cuda as _ccc                     # 退回裸模块


def CAUSAL_CONV1D_FWD(x, w, b, activation_silu: bool):
    """
    统一前向调用，不请求 final_states（final_states_out=None）
    新版(7参): fwd(x, w, b, seq_idx, init_states, final_states_out, activation)
    旧版(5参): fwd(x, w, b, seq_idx, activation)
    """
    try:
        return _ccc.causal_conv1d_fwd(x, w, b, None, None, None, activation_silu)
    except TypeError:
        return _ccc.causal_conv1d_fwd(x, w, b, None, activation_silu)


def CAUSAL_CONV1D_BWD(x, w, b, dy, dx_prealloc, is_causal: bool):
    """
    统一反向包装：
    - 永远不请求 dfinal_states（传 None）
    - 兼容 7 参 / 10 参 / 5 参三套签名
    - 不管底层返回多少个张量，都只取前 3 个 (dx, dw, db)
    """
    def _first3(ret):
        # 某些版本返回 list/tuple （可能含 dfinal_states 等），统一只取前三个
        if isinstance(ret, (tuple, list)):
            return ret[0], ret[1], ret[2]
        # 极少数直接返回的是 3 元组张量（与上面等价）
        dx, dw, db = ret
        return dx, dw, db

    # ① 老 7参：bwd(x, w, b, dy, seq_idx, dx, is_causal)
    try:
        out = _ccc.causal_conv1d_bwd(x, w, b, dy, None, dx_prealloc, is_causal)
        return _first3(out)
    except TypeError:
        pass

    # ② 新 10参常见签名：bwd(x,w,b,dy, seq_idx, init_states, dfinal_states, dx, is_causal, channel_last)
    try:
        out = _ccc.causal_conv1d_bwd(
            x, w, b, dy,
            None,       # seq_idx
            None,       # init_states
            None,       # dfinal_states（不用）
            dx_prealloc,# dx（预分配）
            is_causal,
            False       # channel_last=False
        )
        return _first3(out)
    except RuntimeError as e:
        # 有些实现把 dx/dfinal_states 顺序对调；一旦误把 dx 传给 dfinal_states，就会报 channel-last 限制
        if "dfinal_states is only supported for channel last layout" in str(e):
            out = _ccc.causal_conv1d_bwd(
                x, w, b, dy,
                None,       # seq_idx
                None,       # init_states
                dx_prealloc,# dfinal_states（换位尝试）
                None,       # dx（不用）
                is_causal,
                False
            )
            return _first3(out)
        # 其他 RuntimeError 原样抛出
        raise
    except TypeError:
        pass

    # ③ 最老 5参：bwd(x, w, b, dy, is_causal)
    dx, dw, db = _ccc.causal_conv1d_bwd(x, w, b, dy, is_causal)
    return dx, dw, db
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        conv1d_out = CAUSAL_CONV1D_FWD(x, conv1d_weight, conv1d_bias, True)
    ...
    def backward(ctx, dout):
        ...
        dx, dconv1d_weight, dconv1d_bias = CAUSAL_CONV1D_BWD(
